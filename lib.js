...
const regexParserFactory = (regex, regexType) =>
    new Parser(parserState => {
        const { targetStr, index, isError } = parserState;
        if (isError) {
            return parserState;
        }
        const slicedTarget = targetStr.slice(index);
        ...
        return updateParserError(
            parserState,
            `${regexType}: Couldn't match ${regexType} at index ${index}`
        );
    });
...
const str = s =>
    new Parser(parserState => {
        ...
        return updateParserError(
            parserState,
            `Tried to match ${s} but got ${targetStr.slice(index, index + 10)}`
        );
    });
...
const sepBy = separatorParser => valueParser =>
    new Parser(parserState => {
        ...
        let nextState = parserState;
        while (true) {
            const thingWeWantState = valueParser.parseStateTransformFn(nextState);
...
//             }
//             nextState = nextParser.parseStateTransformFn(nextState);
//             results.push(nextState.result);
//             yieldRes = gen.next(nextState);
//         }
//         return updateParserResult(nextState, results);
//     });
